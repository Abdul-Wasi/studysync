                ...
                {discussions.length === 0 ? (
                    ...
                    discussions.map((discussion) => (
                        <Link to={`/forum/${discussion.id}`} key={discussion.id} className="discussion-card">
                            ...
                            <div
                                className="discussion-snippet quill-content"
                            ...
                            <div className="discussion-meta">
                                ...
                                <span>
                                    {discussion.lastActivityAt ?
                                        new Date(discussion.lastActivityAt?.val || discussion.lastActivityAt).toLocaleString() :
                                        (discussion.createdAt ? new Date(discussion.createdAt?.val || discussion.createdAt).toLocaleString() : 'N/A')
                                    }
                                </span>
                                <span>Comments: {discussion.replyCount || 0}</span> {/* Renamed from Replies to Comments */}
                                <span>Likes: {discussion.likesCount || 0}</span>
                            </div>
                        </Link>
                    ))
                )}
